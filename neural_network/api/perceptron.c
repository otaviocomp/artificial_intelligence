...
int main(){
	float sum, weights[3] = {0,0,0};
	int error, i, input1, input2, epochs, res;
	...
	for(epochs; epochs--;)
		for(i = 4; i--;){
			//sum weights sinapticos e inputs(data de treinamento)
			sum = 0;
			...
			//atualiza os weights(bias and step values are equal to 1)
			if(error){
				weights[0] += error;				
